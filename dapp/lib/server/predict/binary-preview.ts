...
function debugFromPreview(preview: BudgetedTradePreview, reason: string): SideDebug {
    return {
        ...
        decodedMintCost:
            readDebugString(preview.debug, "decodedMintCost") ?? preview.mintCost.toString(),
        decodedRedeemPayout:
            readDebugString(preview.debug, "decodedRedeemPayout") ??
            preview.redeemPayout.toString(),
    };
}
...
function successResponse(preview: BudgetedTradePreview): SideSuccessResponse {
    return {
        ...
        liveOdds: formatBinaryOddsFromQuantity(preview.quantity, preview.mintCost).replace(
            /x$/,
            "",
        ),
...
function failureResponse(error: string, reason: string, caught?: unknown): SideFailureResponse {
    ...
    return {
        ...
        debug: {
            reason,
            devInspectError:
                details?.devInspectError ?? (caught instanceof Error ? caught.message : null),
...
export async function handleBinaryPreviewPost(
    request: Request,
): Promise<NextResponse<PreviewResponse | PreviewErrorResponse>> {
