...
import { previewRangeTradeAmountsServerOnly } from "@/lib/predict-binary/client";
...
async function selectWidth(body: RangeWidthRequest): Promise<RangeWidthData> {
    ...
    const probBps = selectedQuote ? rangeProbabilityBps(selectedQuote) : selection.probabilityBps;
...
export async function handleRangeWidthPost(
    request: Request,
): Promise<NextResponse<RangeWidthResponse>> {
