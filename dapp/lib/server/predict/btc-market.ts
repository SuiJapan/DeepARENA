...
const DEFAULT_PREVIEW_BET_AMOUNT_ATOMIC = (
    10n *
    10n ** BigInt(PREDICT_BINARY_CONFIG.quoteDecimals)
).toString();
...
type FetchResult<T> = { ok: true; value: T } | { ok: false; message: string; debug?: unknown };
...
function readFlexibleField(value: Record<string, unknown>, fieldNames: readonly string[]): unknown {
...
function parseSettlementPriceFields(
...
function parseOracleObjectStateWithFallback(
    value: unknown,
    ...
    for (let page = 0; page < 20; page += 1) {
        ...
        for (const item of result.data) {
            ...
            if (underlyingAsset === PREDICT_BINARY_CONFIG.underlyingAsset) {
                const distance =
                    expiryMs === null ? null : BigInt(Math.abs(expiryMs - targetOracle.expiryMs));
                if (
                    !fallbackCandidate ||
...
export async function handleBtcMarketGet(): Promise<NextResponse<MarketResponse>> {
    ...
    try {
        ...
        const { currentOracle, previousOracle, nextOracle } = selectCurrentBtcOracle(
            oracles,
            nowMs,
        );
        ...
        const [
            ...
            safeFetch(() =>
                fetchOracleObjectState(previousOracle.oracleId, previousOracle.lifecycle),
            ),
        ]);
        ...
        const objectPreviousOracleState = previousOracleObjectStateResult.ok
            ...
            : null;
        const previousOracleState = predictServerPreviousOracleState?.settlementPriceRaw
        ...
        const previousOracleResponse = toPreviousOracleResponse(
            previousOracle,
            previousOracleState,
        );
        ...
        try {
            ...
            return NextResponse.json(
                roundDataErrorResponse({
                    ...
                    originalErrorMessage: caught instanceof Error ? caught.message : String(caught),
                }),
            );
        }
